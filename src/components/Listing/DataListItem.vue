...
<b-card @click.prevent="toggleItem($event,data.id)" :class="{'d-flex flex-row':true,'active' : selectedItems.includes(data.id)}" no-body>
    <div class="pl-2 d-flex flex-grow-1 min-width-zero">
        <div class="card-body align-self-center d-flex flex-column flex-lg-row justify-content-between min-width-zero align-items-lg-center">
            ...
            <p v-if="(data.address == null)" class="mb-0 text-muted text-small w-15 w-sm-100">N/A</p>
            <p v-else-if="(data.address != null)" class="mb-0 text-muted text-small w-15 w-sm-100">{{data.address}}</p>
            <p v-if="(data.price == null)" class="mb-0 text-muted text-small w-15 w-sm-100">N/A</p>
            <p v-else-if="(data.price != null)" class="mb-0 text-muted text-small w-15 w-sm-100">{{data.price}}</p>
            <p v-if="(data.agentAssigned == null)" class="mb-0  text-small w-15 w-sm-100" style="color: #3757FA;">N/A</p>
            <p v-else-if="(data.agentAssigned != null)" class="mb-0 text-small w-15 w-sm-100" style="color: #3757FA;">{{data.agentAssigned}}</p>
            <p v-if="(data.updateAt == null)" class="mb-0 text-muted text-small w-15 w-sm-100">N/A</p>
            <p v-else-if="(data.updateAt != null)" class="mb-0 text-muted text-small w-15 w-sm-100">{{data.updateAt}}</p>
            <p v-if="(data.createdDate == null)" class="mb-0 text-muted text-small w-15 w-sm-100">N/A</p>
            <p v-else-if="(data.createdDate != null)" class="mb-0 text-muted text-small w-15 w-sm-100">{{data.createdDate}}</p>
            <p class="mb-0 text-small w-15 w-sm-100" style="color: #3755F2;">{{data.user.roles}}</p>
